<template>
  <div>
    <h2 id="page-heading" data-cy="PostHeading">
      <span v-text="t$('blogApp.post.home.title')" id="post-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('blogApp.post.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'PostCreate' }" custom v-slot="{ navigate }">
          <button @click="navigate" id="jh-create-entity" data-cy="entityCreateButton" class="btn btn-primary jh-create-entity create-post">
            ...
            <span v-text="t$('blogApp.post.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && posts && posts.length === 0">
      <span v-text="t$('blogApp.post.home.notFound')"></span>
    </div>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span id="blogApp.post.delete.question" data-cy="postDeleteDialogHeading" v-text="t$('entity.delete.title')"></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-post-heading" v-text="t$('blogApp.post.delete.question', { id: removeId })"></p>
      </div>
