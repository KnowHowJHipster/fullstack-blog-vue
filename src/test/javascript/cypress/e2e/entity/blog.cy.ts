...
describe('Blog e2e test', () => {
  ...
  const blogSample = { name: 'besides', handle: 'wiry more chap' };
  ...
  describe('new Blog page', () => {
    ...
    it('should create an instance of Blog', () => {
      cy.get(`[data-cy="name"]`).type('upon');
      cy.get(`[data-cy="name"]`).should('have.value', 'upon');

      cy.get(`[data-cy="handle"]`).type('regarding where pair');
      cy.get(`[data-cy="handle"]`).should('have.value', 'regarding where pair');
